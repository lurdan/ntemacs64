...
DEPPKGS=$(echo mingw-w64-x86_64-{toolchain,xpm-nox,libtiff,giflib,jbigkit,libpng,libjpeg-turbo,librsvg,libxml2,gnutls,libgccjit,cairo,lcms2,zlib,jansson,harfbuzz})
DEPPKGS_PDF=$(echo mingw-w64-x86_64-{poppler,imagemagick})

_prepare () {
  pacman --needed --noconfirm --refresh -S base-devel unzip git $DEPPKGS $DEPPKGS_PDF
}
...
#  curl -O http://ftp.gnu.org/gnu/emacs/emacs-${EMACSVER}.tar.gz.sig
  ...
  patch -p 1 < ../emacs-28-disable-w32-ime.diff
...
_build () {
  #autoconf
  ./autogen.sh
  PKG_CONFIG_PATH=/mingw64/lib/pkgconfig/ CFLAGS='-O2 -march=x86-64 -mtune=generic -s -g0' \
